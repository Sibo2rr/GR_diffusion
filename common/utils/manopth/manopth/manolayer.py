    ...
    def forward(self,
        ...
        if self.use_pca or self.joint_rot_mode == 'axisang':
            ...
            th_full_pose = torch.cat([
                ...
                self.th_hands_mean.to(th_full_hand_pose.device) + th_full_hand_pose
            ], 1)
        ...
        if th_betas is None or th_betas.numel() == 1:
            ...
            th_v_shaped = torch.matmul(self.th_shapedirs.to(th_betas.device),
                                       th_betas.transpose(1, 0)).permute(
                                           2, 0, 1) + self.th_v_template.to(th_betas.device)
            th_j = torch.matmul(self.th_J_regressor.to(th_v_shaped), th_v_shaped)
            # th_pose_map should have shape 20x135

        th_v_posed = th_v_shaped + torch.matmul(
            self.th_posedirs.to(th_pose_map.device), th_pose_map.transpose(0, 1)).permute(2, 0, 1)
        ...
        th_rest_shape_h = torch.cat([
            ...
            torch.ones((batch_size, 1, th_v_posed.shape[1]),
                       dtype=th_T.dtype,
                       device=th_T.device).to(th_v_posed.device),
        ], 1)
