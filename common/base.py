...
class Evaluater(Base):
    def __init__(self, test_epoch):
        self.test_epoch = int(test_epoch)
        super(Evaluater, self).__init__(log_name='test_logs.txt')

    def _make_batch_generator(self):
        # data load and construct batch generator
        self.logger.info("Creating dataset...")
        self.test_dataset = eval(cfg.testset)(transforms.ToTensor(), "tr_eval")
        self.batch_generator = DataLoader(dataset=self.test_dataset, batch_size=cfg.num_gpus * cfg.test_batch_size,
                                          shuffle=False, drop_last=True, num_workers=cfg.num_thread, pin_memory=True)
    ...
    def _evaluate(self, outs, cur_sample_idx):
        eval_result = self.test_dataset.eval_evaluate(outs, cur_sample_idx)
        return eval_result
