...
def get_groundtruth(gt_mano_params):
    mano_pose_size = 16 * 3
    mano_layer = mano.layer
    mano_layer = mano_layer.cuda()
    #mano_layer = DataParallel(mano_layer).cuda()
    #mano_layer = mano_layer.to(device)
    gt_mano_shape = gt_mano_params[:, mano_pose_size:]
    gt_mano_pose = gt_mano_params[:, :mano_pose_size].contiguous()
    gt_mano_pose_rotmat = batch_rodrigues(gt_mano_pose.view(-1, 3)).view(-1, 16, 3, 3)
    gt_verts, gt_joints = mano_layer(th_pose_coeffs=gt_mano_pose, th_betas=gt_mano_shape)
